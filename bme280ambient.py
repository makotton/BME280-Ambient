...
class Ambient:
       ...
       def postData(self, temperature, humidity, pressure):
              self.amb = ambient.Ambient(self.channelId, self.writeKey)
              yyyymmddhhmmss = datetime.datetime.now().strftime('%Y/%m/%d %H:%M:%S')
              response = self.amb.send({'created': yyyymmddhhmmss, 'd1': temperature, 'd2': humidity, 'd3': pressure})
              return response
# <-- 
...
if __name__ == '__main__':
        # create instance
       amb = Ambient(CHANNEL_ID, WRITE_KEY) 
        ...
        while True:
                try:
                        ...
                        for thread in threads:
                                thread.join()                                
                        t1 = threading.Thread(target=amb.postData, args=(value['t'],  value['h'], value['p']))
                        t1.start()
                        threads.append(t1)
